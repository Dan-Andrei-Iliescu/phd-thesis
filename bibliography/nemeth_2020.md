...
- The content information in $s_i$ can be measured as the mutual information between it and the other observations in the group $I(q(s_i) || q(\textbf{x}_{-i}))$. This should be minimized.
	- They express the mutual information between data samples and style representations as $$I(x;s) = \text{KL} [r(x,s) || \overline{r}(x,s)] = \mathbb{E}_{r(x,s)} ~ \text{log} \frac{r(x,s)}{\overline{r}(x,s)}$$ where $r(x,s) = r(s|x)r(x)$ and $\overline{r}(x,s) = r(x)r(s)$. We define $$r(s|x_i^n) = \frac{1}{K_n - 1} \sum_{j=1,j \neq i}^{K_n} q(s|x_j^n)$$ Also, it is proven that $r(x) = q(x) = p_D(x)$
...
- They approximate the mutual information using desity-ratio training [[belghazi_2018]] [[nguyen_2010]]. The discriminator $T$ is trained to **maximize** $$\mathbb{E}_{r(x,s)} [T(x,s)] - \text{log} ~ \mathbb{E}_{\overline{r}(x,s)} [e^{T(x,s)}]$$ 

...
\item \change{Then, we use the task of fair comparisons between student test-scores (Figure \ref{fig:trans}) to show that the amount of conditional shift in the dataset determines the performance gap between our model and the rest.}
\end{enumerate}
...
Since the data-generating process is known, the mutual information between the inferred group variable and the ground-truth group variable $I(\mathbf{u}; \mathbf{b}, \mathbf{c})$ is straightforward to implement by following the approach from \citet{Chen2018IsolatingSO}.
...
\begin{table}[t]
\caption{\change{Our CxVAE outperforms existing methods at disentangling between object-colour and spotlight-colour in the 3DIdent dataset \citep{Zimmerman2021Contrastive}. Our model's MIG score is much higher than the one produced by competing models, and the performance gap is greater than the 95\% confidence interval for any one of these models.} \todo{Where are the confidence intervals coming from?}}
\label{table:results}
\begin{center}
\begin{tabular}{llll}
...
COCO-FUNIT \citep{Saito2020Coco} & 0.05 $\pm$ 0.01 & 0.13 $\pm$ 0.04 & 0.13 $\pm$ 0.06\\

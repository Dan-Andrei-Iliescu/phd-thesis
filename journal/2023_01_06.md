...
1. Start by summarising the story of the talk so that people know what the destination is. Maybe offer a teaser, an example showing why bias correction fails when predicting heatwave duration.
2. Clarify what the problem is with Bias Correction, and why your framework helps with that. Define a term, like time-sync, that encapsulates the problem, and repeat it throughout the presentation. Briefly, in order to predict heatwaves, you need a weather simulation. Bias correction can help climate simulations with monthly temperature histograms, but not with the statistics of consecutive time-points. Probabilistic modelling reveals the limitations in the assumptions of these Bias Correction methods. Therefore, we propose a model that makes the right assumptions.
3. When introducing the probabilistic framework, remember to explain that maximum likelihood on the constant is the way to turn the probabilistic models you presented into the existing Bias Correction methods.
4. Change the toy data plots to show the ground-truth on the left and the predicted outputs on the right.
5. The fact that you don’t show real data is not a problem. Just say that what we’re showing is a proof of concept. Show that your model outperforms EQM (or MORA) on toy data, say that this is extremely promising, and then say how excited you are to apply this to real data. This is something THEY can help you with.
